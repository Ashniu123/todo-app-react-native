...
test('should remove data item on remove item', () => {
  ...
  const modifiedState = { data: tasks, prev: initialState.prev };
  const state = TaskReducer(modifiedState, action);
...
test('should remove completed data items on remove completed', () => {
  ...
  const modifiedState = { data: tasks, prev: {} };
  const state = TaskReducer(modifiedState, action);
  const expectedDataValue = tasks.filter((task) => {
    return task.completed === false;
  });
  const expectedPrevValue = tasks.filter((task) => {
    return task.completed === true;
  });
  expect(state.data).toEqual(expectedDataValue);
  expect(state.prev).toEqual(expectedPrevValue);
});
...
test('should restore previously removed item on undo', () => {
  ...
  const state = TaskReducer(modifiedState, action);
  const expectedValue = [...tasks, ...prev];
  expect(state.data).toEqual(expectedValue);
  expect(state.prev).toEqual(initialState.prev);
});
